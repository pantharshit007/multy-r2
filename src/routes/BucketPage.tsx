    ...
    <main className="grid gap-4 lg:grid-cols-[1fr_340px]">
      ...
      <section className="min-w-0 rounded-3xl border border-zinc-800 bg-zinc-950/70 p-5 shadow-2xl">
        <div className="flex items-center justify-between gap-3">
          ...
          <button className="rounded-xl border border-zinc-700 px-3 py-2 text-sm text-zinc-300 hover:border-amber-300" onClick={() => runAction(() => refreshObjects())} type="button" disabled={isPending}>
            Refresh
          </button>
        </div>
    ...
    <aside className="rounded-3xl border border-zinc-800 bg-zinc-950/70 p-5 shadow-2xl lg:sticky lg:top-4 lg:self-start">
      ...
      <form className="mt-4 grid gap-4" onSubmit={save}>
        <label className="grid gap-2 text-sm font-medium text-zinc-300">
          Workers Endpoint
          <input className="h-11 rounded-xl border border-zinc-700 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none focus:border-amber-300" value={form.endPoint} onChange={(event) => setForm({ ...form, endPoint: event.target.value })} required />
        </label>
        <label className="grid gap-2 text-sm font-medium text-zinc-300">
          API Key
          <input className="h-11 rounded-xl border border-zinc-700 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none focus:border-amber-300" value={form.apiKey} onChange={(event) => setForm({ ...form, apiKey: event.target.value })} type="password" required />
        </label>
        <label className="grid gap-2 text-sm font-medium text-zinc-300">
          Custom Domain
          <input className="h-11 rounded-xl border border-zinc-700 bg-zinc-900 px-3 text-sm text-zinc-50 outline-none focus:border-amber-300" value={form.customDomain} onChange={(event) => setForm({ ...form, customDomain: event.target.value })} />
        </label>
